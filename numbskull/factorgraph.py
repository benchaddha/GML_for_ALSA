...
# 
# This module contains the FactorGraph class which represents a factor graph 
# and provides methods for inference and learning.
# ===----------------------------------------------------------------------===//\
...
def run_pool(threadpool, threads, func, args):
    """Run the given function in parallel using a thread pool."""
    if threads == 1:
        func(0, *args)
    else:
        future_to_samples = \
...
class FactorGraph(object):
    ...
    def clear(self):
        """Clear the factor graph and shut down the thread pool."""
        self.count[:] = 0
    ...
    def getWeights(self, weight_copy=0):
        """Get the weights of the factor graph."""
        return self.weight_value[weight_copy][:]

    def getMarginals(self, varIds=None):
        """Get the marginals of the factor graph."""
        if not varIds:
    ...
    def diagnostics(self, epochs):
        """Print diagnostic information about the factor graph."""
    ...
    def diagnosticsLearning(self, weight_copy=0):
        """Print diagnostic information about the learning process."""
    ...
    def burnIn(self, epochs, sample_evidence, diagnostics=False,
        ...
        """Perform burn-in for the factor graph."""
        if diagnostics:
    ...
    def inference(self, burnin_epochs, epochs, sample_evidence=False,
        ...
        """Perform inference for the factor graph."""
        # Burn-in
        if burnin_epochs > 0:
    ...
    def learn(self, burnin_epochs, epochs, stepsize, decay, regularization,
        ...
        """Perform learning for the factor graph."""
        # Burn-in
        if burnin_epochs > 0:
    ...
    def dump_weights(self, fout, weight_copy=0):
        """Dump the weights of the factor graph to a text file."""
        with open(fout, 'w') as out:
            for i, w in enumerate(self.weight):
                out.write('%d %f\n' % (i, self.weight_value[weight_copy][i]))

    def dump_probabilities(self, fout, epochs):
        """Dump the probabilities of the factor graph to a text file."""
        epochs = epochs or 1
        with open(fout, 'w') as out:
